...
class TestBASimConvergence(unittest.TestCase):
    ...
    def test_sim(self): 
        ...
        meas_gap=60*30 #interval between measurements in seconds
        dt = 1
        dt_goal = 1 #timestep in seconds
        ...
        x0 = x0d1
        # x0[0:3] += y*np.random.randn(3,N)
        #Initialize the EKF class
        BA_test = src.BA(x0c, x0,y, dt, meas_gap, dt_goal)

        #iterate the BA

        max_iters = 3 #number of timesteps to run the Batch LSQ
